...
Ray Camera::GenerateRay(const float sx, const float sy) const
{
	...
	//float pixel_s = 0.056;
	//Vector3 origin = Vector3(view_from_);
	////float focal_length = height_ / 2 * tan(fov_y_ / 2);
	//float focal_length = 35.f;
	//float dy = pixel_s * (-y + height_ /2.0 - 0.5f); //protože camera space je obrácenì
	//float dx = pixel_s * (x - width_ / 2.0 + 0.5f);
	//Vector3 direction = Vector3(dx, dy, -focal_length);
	//Vector3 directionWorld = view_t_ * direction;
	//directionWorld.Normalize();
	//return Ray(origin, directionWorld);
}
